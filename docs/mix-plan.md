...
- Phase 1 の安定確認後、`MIXER_ENABLED` フラグと旧経路を削除（将来）

### Phase 6 — 曲ごとの DJ つなぎ 🚧（現行プラン）

詳細は [`docs/mix-transition-phase6.md`](mix-transition-phase6.md)（**PR #27 が正**）。
...
- `stop()` 後は MixStream を作り直し、想定外 Idle では現在曲を再 `playNext()`
