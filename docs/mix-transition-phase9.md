...
### 追記: Codex レビュー対応（PR #46, round 7）— 未対応（保留）

round 6 のプッシュ後、`transitionCandidates.js` の `toCandidate()`（`score: plan.confidence` を直接使う、round 2/3 の根本修正がたどり着いた最終形）に対して、round 2 の指摘の再燃に見える、しかし実際には別種の——かつより根が深い——指摘が入った。

- **P2（未対応）: 現在の設計では stem-mix は beatmix に絶対に負けられない（tie ではなく、数学的に必ず ≥）** — round 2/3 の根本修正は「stem-mix/beatmix それぞれの探索ループ自体が、常に strict（`stemAware: false`）スコアで候補ペアをランキングする」という形で「post-hoc 補正は探索が既に捨てたペアを復元できない」問題を解決した。しかしこの修正の帰結として、`planStemTransition()` の探索が渡る candidate pool（exit: `requireExitVocalSafe:false` で mid-vocal も許可／entry: `requireEntryForwardSafe:false` で forward-safety の room 制約なし）は、`planBeatmixTransition()` を直接呼んだ場合の candidate pool の**真の超集合**になっている。かつ両者は同じ tier セット（extended tier は `stemAware` 前提なので、通常ケースでは両者とも `MIX_BARS.preferred` 以下に限定される）を同じ strict スコア式で探索する。superset の探索が subset の探索より良いスコアを見つけられないことは構造上あり得ない（beatmix が見つけた勝者ペアは stem-mix の pool にも必ず含まれる）ため、`stemMix.confidence >= beatmix.confidence` が**常に**成り立つ。これは `beatmix.score + 0.05 > stemMix.score + 0.10`（beatmix が勝つ条件）が `beatmix.score > stemMix.score + 0.05` を要求し、`beatmix.score <= stemMix.score` と矛盾するため、beatmix はどれだけ品質差があっても stem-mix に**原理的に勝てない**——単なる「僅差では stem-mix が有利」ではなく、§6.4 が明記する「品質差が大きければ beatmix が選ばれ得る」という除外規定がこの設計のもとでは到達不能になっている。
  - なぜこのセッションで即座に修正しなかったか: 素朴な「stem-mix にペナルティを足す」post-hoc 補正は、それが勝者ペア選択自体に影響しない限り round 2 の同じ轍は踏まないが（探索自体は既に strict スコアで正しく最良ペアを選んでいるので、「別ペアを復元できない」問題とは別種）、ペナルティの**大きさ**を経験的根拠なしに決め打ちすることになる。何を測っていないから beatmix が理論上勝てて然るべきなのかというと、「選ばれたペアが beatmix 自身のゲート（vocal-safety/forward-safety）では許可されない relaxation を必要としたかどうか」であり、現在の strict スコアはこれを一切減点していない（意図的に——relaxation が必要だったかどうかに関わらず同じペアは同じ strict スコアになるべき、というのが round 2 の設計だった）。
  - PR コメントで2つの方向性を提案し保留とした: (1) stem-mix の勝者ペアが実際に beatmix 自身のゲートでは通らない relaxation を必要とした場合のみペナルティを課す（beatmix でも合法だったペアには課さない——「品質が本当に同じ場合は tie で stem-mix 優先」という round 2 の意図は保つ）。(2) mode レベルの定数ペナルティではなく、relaxed exit がどれだけ vocal の奥まで踏み込んでいるかに応じた連続的な品質シグナルをスコア自体に組み込む（`vocalSafety` が本来担うべきリスクをそのまま反映させる）。
  - 現状: 未対応。`docs/mix-transition-phase9.md` 本ノートへの記載と PR コメントでの診断のみ。
