...
**デモログイン機能**: Google OAuth 審査担当者がDiscordアカウントを作成せずに YouTube 同意画面に到達するための限定的なログイン機能。`DEMO_LOGIN_ENABLED` / `DEMO_LOGIN_PASSWORD`（`.env` のみ、`DEMO_LOGIN_PASSWORD` が未設定なら自動的に無効）で制御される。非公開の `/login/demo` ページからパスワードを入力すると、固定ID `google-review-demo` のデモユーザーとしてセッションが発行され、ダッシュボード表示・YouTube OAuth 連携・プレイリスト閲覧といった審査に必要な導線を辿れる。セキュリティ根拠として、デモユーザーは実 Discord ギルドに所属しないため、実 Bot 操作時に必須の `requireBotPermission()` ギルド権限チェック（`src/web/server/routes/route-utils.js`）により自然に 403 で遮断される。追加のアクセス制御コードは不要。**審査完了後は `DEMO_LOGIN_ENABLED` を外す（または `false` にする）ことで即座に無効化できる**。無効化時は起動時のクリーンアップとして `web_sessions`・`service_links` の `discord_id='google-review-demo'` 行を自動削除するため、既存のデモセッションや連携済み YouTube トークンがそのまま使われ続けることはない。また、デモアカウントは全審査担当者が同じ固定IDを共有する設計のため、`service_links` の連携情報は担当者間で使い回されると前任者の非公開プレイリストが次のログインから閲覧可能になってしまう。これを防ぐため、デモログイン成功のたびにも既存の `service_links` 行を削除してから新規セッションを発行する。手動クリーンアップとして残るのは `discord_users` テーブルの行のみで、削除すればデモアカウント関連のデータを完全に除去できる。
