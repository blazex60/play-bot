...
// --- Phase 9I (docs/mix-transition-phase9.md §11): incoming vocal independent timeline ---

test('acceptance (mixer): incoming vocal stem seeks independently of instrumental, backward from its own phrase start by the hold duration', async () => {
  // §11.1's problem: today, incoming vocal opens at the SAME startSec as
  // instrumental and is read every tick from crossfade-tick-0 regardless of
  // gain, so what's actually audible once it fades in is whatever native
  // content landed `holdSec` PAST that shared startSec — arbitrary relative
  // to the incoming track's own vocal phrasing. This proves the fix:
  // instrumental keeps the plan's entrySec, but vocal seeks to
  // firstVocalStartSec - holdSec instead, so it lands exactly ON the
  // phrase's own natural start the instant gain ramps up.
  const frame = Buffer.alloc(FRAME_BYTES);
  new Int16Array(frame.buffer).fill(4000);
  const { outgoingAnalysis, incomingAnalysis } = stemFixtures();
  // Shrink the outgoing vocal tail so the hold+release (and therefore
  // inVocal's own delay) works out to a clean, easy-to-verify number:
  // exit(1.0s) -> lastVocalEndSec(2.0s) = 1.0s hold, +0.5s release (room:
  // fadeSec(8s) - hold(1.0s) = 7s, plenty), +0.2s DEFAULT_VOCAL_CROSSOVER_
  // MARGIN_SEC = 1.7s total inVocalDelaySec. Same bpm on both sides (120)
  // keeps sessionTempo.tempoRatio at 1, so no native/playback conversion
  // complicates the expected value.
  const outgoing = { ...outgoingAnalysis, lastVocalEndSec: 2.0 };
  const incoming = { ...incomingAnalysis, firstVocalStartSec: 10.0 };

  const stemSourceCalls = [];
  const { player, queue } = makePlayer({
    trackDuration: 8,
    track: createTrack({ title: 'Track A', webpageUrl: 'https://example.com/a', duration: 8, videoId: 'vid-a' }),
    getTrackAnalysisFn: async (videoId) => (videoId === 'vid-a' ? outgoing : incoming),
    analyzeTrackFileFn: null,
    probeTempoBackendFn: async () => 'rubberband',
    createPcmSourceFn: async () => PcmSource.fromBuffers(Array.from({ length: 400 }, () => Buffer.from(frame))),
    getCachedStemsFn: async (videoId) => ({
  ...
  try {
    ...
    assert.ok(startedPlan, 'expected a crossfade to have armed');
    assert.equal(startedPlan.mode, 'stem-mix');
    // test invariant: confirms the hand-derived 1.7s inVocalDelaySec above.
    assert.ok(Math.abs(startedPlan.stems.inVocal.startOffsetSec - 1.7) < 1e-9,
      `test invariant: expected inVocal.startOffsetSec ~= 1.7, got ${startedPlan.stems.inVocal.startOffsetSec}`);

    const incomingVocalCall = stemSourceCalls.find((c) => c.filePath === '/tmp/vid-b.vocal.wav');
    const incomingInstCall = stemSourceCalls.find((c) => c.filePath === '/tmp/vid-b.instrumental.wav');
    assert.ok(incomingVocalCall, 'expected an incoming vocal stem source to have been spawned');
    assert.ok(incomingInstCall, 'expected an incoming instrumental stem source to have been spawned');

    // instrumental: unchanged from Phase 8 — the plan's own entrySec (0.2s).
    assert.ok(Math.abs(incomingInstCall.opts.startSec - 0.2) < 1e-9);
    // vocal: firstVocalStartSec(10.0) - holdSec(1.7) = 8.3s, NOT entrySec.
    assert.ok(Math.abs(incomingVocalCall.opts.startSec - 8.3) < 1e-9,
      `expected the vocal stem to seek to firstVocalStartSec - hold (8.3), got ${incomingVocalCall.opts.startSec}`);
    assert.notEqual(incomingVocalCall.opts.startSec, incomingInstCall.opts.startSec,
      'vocal and instrumental must seek independently, not share one startSec');
  } finally {
    await player.stop();
  }
});
