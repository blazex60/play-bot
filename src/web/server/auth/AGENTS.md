...
Discord / Spotify / YouTube の OAuth 2.0 authorize + callback route と、それらが共有する state/PKCE/token ヘルパー。
...
| `spotify.js` | `registerSpotifyAuthRoutes` — プレイリスト読み取り用 OAuth（`playlist-read-private playlist-read-collaborative`）。取得トークンは `tokenStore.js` 経由で暗号化保存。**UI からは disabled**（`web/src/components/PlaylistPanel.jsx` 参照）だが backend route はそのまま生きている |
...
- Spotify の client secret 等、OAuth client credential は `config.js` 経由でのみ参照する

### Testing Requirements
- `discord.test.js`, `spotify.test.js`, `youtube.test.js` がある。`fetchImpl` を差し替え可能にして外部 HTTP 呼び出しをモックする設計に揃える
