...
ビルドとテスト実行を仲介するスクリプト群。`package.json` の bun scripts から呼ばれる薄いラッパーと、QA タスクをサンドボックス化して実行する manifest ベースのランナー。
...
### Internal
- `qa-task.mjs` は `qa-manifest.mjs` と `qa-safety.mjs` に依存し、`bun-cli.mjs` の `assertSupportedBunVersion` を再利用する
