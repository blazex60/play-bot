...
const SETTINGS_COMMANDS = [
  ...
  { title: '/normalize <enabled>', copy: 'サーバー単位の音量ノーマライズ設定を保存します。PCM ミキサー経路ではクロスフェード品質のため、尺が分かる30分以内の曲には常に loudnorm を適用します（無効化しても再生経路では外れません）。尺不明（ライブ等）や30分超の曲はストリーム再生になります。' },
  { title: '/fade <enabled>', copy: '曲間のフェード（クロスフェード / tail-fade）を有効・無効にします。サーバー単位の設定で、無効にすると曲の切り替わりはギャップレスになります。' },
